...
timer_sleep(int64_t ticks)
{
    ...
    list_insert_ordered(&sleep_list, &cur->block_elem, thread_wakeup_time_less, NULL);
...
static bool sleep_less_func(const struct list_elem* a,
    ...
    void* aux UNUSED)
{
    struct thread* ta = list_entry(a, struct thread, block_elem);
    struct thread* tb = list_entry(b, struct thread, block_elem);
...
static void timer_wakeup(void)
{
    ...
    {
        struct thread* t = list_entry(list_front(&sleep_list), struct thread, block_elem);
